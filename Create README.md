...
<img src="report-source/charts/chart_outcome_breakdown.png" width="420">
Full methodology, benchmarks, and ROI math: AP_Invoice_Automation_Capstone_Report.pdf
...
Renders synthetic invoices as images (3 visual layouts + simulated scan noise) — not clean structured data
Reads them directly with Claude's vision model — no per-vendor OCR templates
Validates, matches against purchase orders (3% tolerance), and routes exceptions
Auto-approves clean matches; posts to a mock ledger
Reports live KPIs: touchless rate, extraction accuracy, exception breakdown
...
prototype/dashboard.html calls Claude's API to read each invoice image. That call only works when the file is opened as an artifact inside Claude.ai — the platform proxies the request without an exposed API key.
...
Project structure
AP-Invoice-Automation-Capstone/
├── README.md
...
└── report-source/                              scripts that regenerate the PDF report
    ├── generate_charts.py
    ├── build_report.py
    └── charts/
Business rules
